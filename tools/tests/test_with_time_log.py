...
def compute_on_dataset(model, data_loader, device, timer=None, show=False):
    ...
    # Cost time log of each batch times
    time_cost_log = []
    ...
    for i, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            time_cost_log.append(time.time() - time_start)
    ...
    # Set assert to ensure test inference time in batch_size=1
    assert len(time_cost_log) == len(data_loader.dataset), \
           "When test each frame inference time cost, must set samples_per_gpu=1 " + \
           "in configs/deepcamp_all_vfev3_spmiddlefhd_rpn1_mghead_syncbn.py."
    print("\n\nInference Time: ")
    # Discard front 200 frames which time cost may not stabe
    time_cost_log = np.array(time_cost_log[200:]) * 1000.
    strr = 'Mean: %.0f ms, Min: %.0f ms, Max: %.0f ms, Mid: %.0f ms.'%(
            np.mean(time_cost_log), 
            np.min(time_cost_log), 
            np.max(time_cost_log),
            np.median(time_cost_log))
    # Print time analysis log
    print(strr + "\nInfenence End\n\n")
